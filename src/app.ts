...
if (!btnSucess) throw new Error("AddTodo button not found");

const btnSubmit = document.getElementById("button_submit")
if (!(btnSubmit instanceof HTMLInputElement)) throw  new Error("Submit not found");
...
//delete specific row
function deleteTask(dId: number){
...
form.addEventListener("submit", (e) => {
    ...
    if(!indicator) {
        ...
        if (!row) throw new Error("There is no task");
        row.innerHTML = `
                            <td>${lists.title}</td>
                            <td>${lists.task}</td>
                            <td>${lists.date}</td>
